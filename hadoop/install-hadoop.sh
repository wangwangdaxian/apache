...
function download() {
  curl -s "http://mirrors.tuna.tsinghua.edu.cn/apache/hadoop/common/$version/$version.tar.gz" -o "$version.tar.gz"
  tar -zxf "$version.tar.gz" -C "./$version"
  sudo mv "./$version" /usr/local
